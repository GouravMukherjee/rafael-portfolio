...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    // Desktop items — animate each child directly (avoids display:none parent issue)
    if (desktop) {
    ...
    >
      ...
      >
        {galleryItems.map((item) => (
          <GalleryItem key={item.id} item={item} containerStyle={item.gridStyle} />
        ))}
      </div>
      ...
      >
        {galleryItems.map((item) => (
          <GalleryItem key={`m-${item.id}`} item={item} containerStyle={item.mobileStyle} />
        ))}
      </div>
    </section>
  );
